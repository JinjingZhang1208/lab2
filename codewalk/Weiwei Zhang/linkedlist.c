...
node_t* create_node(int year_win, int number_win) {
    node_t* new_node = (node_t*)malloc(sizeof(node_t));
    if (new_node == NULL) {
        printf("Memory allocation failed\n");
        exit(1);
    }
    new_node->years = year_win;
    new_node->numWins = number_win;
    new_node->next = NULL;
    return new_node;
}
...
node_t* create_list(int* scores, int* years, int length) {
    ...
    for (int i = 0; i < length; i++) {
        node_t* new_node = create_node(years[i], scores[i]);
        // insert new node at the the beginning
        new_node->next = head;
...
void free_list(node_t* list) {
    //TODO complete me!
    while (list != NULL) {
	// store the pointer of next node to iterator

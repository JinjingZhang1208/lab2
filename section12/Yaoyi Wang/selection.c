...
int findMinimum(int* array, int start, int stop){
    // Iterate through a subset of the array and find the minimum value.
    // Return the index of that minimum value.

   // initialize 'minIndex' with the starting index.
   int minIndex = start; 
   // Iterate through the array, starting from 'start + 1' up to 'stop - 1'.
   for (int i = start + 1; i < stop; i++) {
        // Compare the current element with the current minimum.
        if (array[i] < array[minIndex]) {
            // If a new minimum is found, update 'minIndex' with the index
            // of the new minimum
            minIndex = i;
        }
   }

...
void bubblesort(int* array, unsigned int size) {
    // Outer loop that runs 'size - 1' times.
    // This loop shows how the largest unsorted element
    // is bubbled up after each pass over the array.
    for (int i = 0; i < size - 1; i++) {
        // Inner loop that iterates through the unsorted part of the array.
        // As 'i' increases, the number of elements to check
        // decreases as the array's end is sorted. 
        for (int j = 0; j < size - 1; j++) {
            // Compare adjacent elements.
            // If the current element 'array[j] is greater than
            // the next 'array[j+1], they need to be swapped.
            if (array[j] > array[j + 1]) {
                // Swap the current element with the next element.
                // The 'swap' function swaps the elements
                // in addresses 'j' and'j+1'.

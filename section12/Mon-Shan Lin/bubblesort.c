...
void bubblesort(int* array, unsigned int size) {
    // Traverse the array n times (n is the size of array)   
    for (int i = 0; i < size - 1; i++) {
        // Iterate each number in the array  
        for (int j = 0; j < size - 1; j++) {
            // For every adjacent pairs, if the number is larger than the next number in the array
            if (array[j] > array[j + 1]) {
                //  The two numbers swap their positions in the array

...
void generateGraph(int* g) {
	...
	for (i = 0; i < ROWS; i++) {
		for (j = 0; j < COLUMNS; j++) {
			if (i == j) {
				g[i * ROWS + j] =
				    0;	// Why do we have this condition?
			} else {
				g[i * ROWS + j] = rand() % 2;
			}
		}
...
int nodeInDegree(int* g, int node) {
	...
	return count;
	// remove this line, it is present so the code compiles
}
...
int isConnected(int* g, int node1, int node2) {
	...
	if (node1 < 0 || node1 >= ROWS || node1 >= COLUMNS || node2 < 0 ||
	    node2 >= COLUMNS || node2 >= ROWS) {
		return -1;
	}
	if (g[node1 * COLUMNS + node2] == 1) {
		return 1;
	}
	/*	if (g[node2 * COLUNMS + node1] == 1) {
			return 1;
		} */
	return 0;
}

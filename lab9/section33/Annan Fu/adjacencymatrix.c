...
int nodeInDegree(int* g, int node) {
	...
	return inDegree;  // remove this line, it is present so the code
			  // compiles
}
...
int nodeOutDegree(int* g, int node) {
	...
	outDegree = 0;
	for (i = 0; i < ROWS; i++) {
		if (g[COLUMNS * node + i] == 1) {
			outDegree++;
		}
	}
	return outDegree;  // remove this line, it is present so the code
			   // compiles
}

...
int nodeInDegree(int* g, int node) {
	...
	for(startingPoint = 0; startingPoint < ROWS; startingPoint ++){
		if(g[startingPoint][endingPoint] = 1){
		countEdge ++;}
...
int nodeOutDegree(int* g, int node) {
    // TODO
	int startingPoint = node;
	int edingPoint;
	int countEdge = 0;

	for(endingPoint = 0; endingPoint < COLUMNS; endingPoint ++){
		if(g[startingPoint][endingPoint] = 1){
		countEdge ++;}

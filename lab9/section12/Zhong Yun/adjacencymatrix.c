...
int nodeInDegree(int* g, int node) {
    ...
    for (int i = 0; i < ROWS; i++) {
        // If the value at the index is 1, increment the in-degree count
        if (g[i * ROWS + node] == 1) {
            in_degree_count++;
        }
    }
...
int nodeOutDegree(int* g, int node) {
    ...
    for (int i = 0; i < COLUMNS; i++) {
        // If the value at the index is 1, increment the out-degree count
        if (g[node * ROWS + i] == 1) {
            out_degree_count++;
        }
    }
...
int isConnected(int* g, int node1, int node2) {
    // The graph is represented as a 1D array, so we need to calculate the index
    int index = node1 * COLUMNS + node2;

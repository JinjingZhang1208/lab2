...
// Name: nodeInDegree
// Inputs:
// - g: pointer to the graph (shown as adjacency matrix)
// - node: the given node 'n' in the adjacency matrix
// Output:
// - int, number of in-degrees of a node
int nodeInDegree(int* g, int node) {
    int inDegree = 0;
    int i;
    for (i = 0; i < ROWS; i++) {
        inDegree += g[i * ROWS + node];
    }
    return inDegree;
}
...
// - node: the given node 'n' in the adjacency matrix
// Output:
// - int, number of out-degrees of a node
int nodeOutDegree(int* g, int node) {
    int outDegree = 0;
    int i;
    for (i = 0; i < COLUMNS; i++) {
        outDegree += g[node * ROWS + i];
    }
    return outDegree;
}
...
// Inputs:
// - g: pointer to the graph (shown as adjacency matrix)
// - node1: the given source node 1 to check if there is connection between node 1 and node 2
// - node2: the given destination node 2 to check if there is connection between node 1 and node 2
// Output:

...
//  - 'left' is the starting index of the array to be partitioned.
//  - 'right' is the ending index of the array to be partitioned.
// Output: The function returns an integer representing the partition index where the pivot element is finally placed.
// Description: This function takes the last element as pivot, places all smaller than pivot to left and all greater to right

int partition(int arr[], int low, int high){
    // set the last element to be the pivot.
    int pivot = arr[high];
    // initialize an index i to one postion before left index.
    int i = (low - 1);

    // Iterate through the array from the left index up to one before the pivot.
    for (int j = low; j <= high - 1; j++){
        // Condition: if the current element is smaller than the pivot.
        if(arr[j] < pivot){
            // move the index i to the right.
            i++;
            // swap the elements.
            int temp = arr[i];
            arr[i] = arr[j];
    ...
    // place the pivot to its correct postion.
    // After the loop, swap the element following the last element that is smaller than the pivot with the pivot.
    int temp = arr[i + 1];
    arr[i + 1] = arr[high];
    arr[high] = temp;
    // Pivot in its correct postion, return the index of the pivot.
    return (i + 1);
}
...
// Input(s):
//  - 'arr' is the input array of integers.
//  - 'low' is the starting index of the array to be sorted.
//  - 'high' is the ending index of the array to be sorted.
// Output: No value is returned, but the 'arr' array between indices 'low' and 'high' is sorted in ascending order.
// Description: This function selects a pivot element and partitions the array around the pivot using the partition function.
void quickSort(int arr[], int low, int high){
    // Check if the lower index is less than the higher index
    if(low < high){
        // call the partition function to find the correct postion for the pivot
        int pivot = partition(arr, low, high);
        // Recursively apply quickSort on the left side of the pivot
        quickSort(arr, low, pivot - 1);
        // Recursively apply quickSort on the right side of the pivot
        quickSort(arr, pivot + 1, high);
    }
    // If the condiction: low < high is false, then no need to swap elements
}
...
void sortIntegers(int* array, unsigned int size) {
    ...
    // Call quickSort function on the input array

...
int partition(int arr[], int low, int high) {
    ...
    int i = (low - 1); //i be the smaller element

    for (int j = low; j <= high - 1; j++) {
        // If current element is smaller than or equal to the pivot
        if (arr[j] <= p) {
            i++; //Increase the value of i
	    //The following there lines of code implemented the swap.
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
        }
...
void quickSort(int arr[], int low, int high) {
    if (low < high) { // If there are at least one element
        int pIndex = partition(arr, low, high);// Partition the array
        quickSort(arr, low, pIndex - 1);// Recursively sort the array left of the pivot
        quickSort(arr, pIndex + 1, high);//Recursively sort the array right of the pivot 
...
void sortIntegers(int* array, unsigned int size) {
    ...
    quickSort(array, 0, size - 1); //Call the quickSort function to sort the whole array.
}

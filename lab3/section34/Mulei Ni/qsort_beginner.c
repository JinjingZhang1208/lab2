...
{
    // Set the last element to the pivot
    int pivot = arr[high];
    // Initialazation a poionter
    int i = low - 1;
    ...
    for (int j = low; j < high; j++)
    {
        if (arr[j] < pivot)
        {
            // Increment i to the further swap
            i++;

            // Swap the two element make the smaller value in preceding place.
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
        }
...
void quickSort(int arr[], int low, int high)
{
    if (low < high)
    {
        // Get the pivot position from partition
        int pivot = partition(arr, low, high);
        
		// Using the quick sort to sorting the left part to the pivot
		quickSort(arr, low, pivot - 1);
       
		// Using the quick sort to sorting the right part to the pivot

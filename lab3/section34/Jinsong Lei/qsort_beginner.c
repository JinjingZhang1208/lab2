 ...
 * Below is partition function for quick sort
 * Name: partition
 * Inputs:
    - 'array' is the array being partitioned
    - 'low' is the starting index
    - 'high' is the ending index
 * Output: pivot index */
 int partition(int arr[], int low, int high) {
    int i = low - 1; // we create a variable i as a moving point that is on index 0 - 1
    int j = low; // we create another varibale as a moving point that starts at index 0
    int pivot = arr[high]; // we always choose the last number of array as our pivot in the Lumoto method
    while (j <= high - 1) { // we set the condition so that index j does not overlap with the pivot
        if (arr[j] <= pivot) { // If the number under index j is less than or equal to the pivot 
            i++; // we first increment i by 1
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp; // then we swap the number under i and the number under j
        }
        j++; //after one round, we increment j by 1 to continue the loop logic
    }
    int temp = arr[i+1]; 
    arr[i+1] = arr[high];
    arr[high] = temp; // after we exit the while loop, we swap the number under i+1 and our pivot
    return i + 1; // we return the pivot after swap index i+1 in the end
 }

/* Below is quicksort function for quick sort
 * Name: quicksort
 * Inputs:
    - 'array' is the array being quick sorted
    - 'low' is the starting index
    - 'high' is the ending index
 * Output: none */

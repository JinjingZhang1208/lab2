...
XINYI WANG
CS 5008, Spring 2024
Lab3: Quicksort and Divide & Conquer
*/

// Compile with: clang -g qsort_dataset3.c -o qsortb
// Run with: ./qsortb
...
int partition(int arr[],int low, int high){
  int pivot = arr[high];// choose the last element as the pivit, set rightmost index as pivot index
  int i =(low-1); //pointer for the element
  ...
  if (arr[j] <= pivot)
{
   ...
   // swap it with the greater element pointed by i,increment index if smaller element
    i++;
   //swap element at i with element at j,without using the swap function
   int temp = arr [i];
   arr[i]= arr[j];
   arr[j]=temp;
   }
 ...
 //swap(&array[i+1],&array[high]);  if using the swap function
  int temp = arr[i+1];
  arr[i+1]=arr[high];
  arr[high]=temp;
  return (i+1);
...
void quickSort(int arr[],int low, int high){
 if (low < high){
  ...
  //elements smaller than the pivot are the left side of pivot
  //elements greater than the pivot are on the right side of the pivot
  //pi means the index of partition
  int pi = partition( arr,low, high);

  //recursive call on the left of pivot
  quickSort(arr,low, pi-1);
  printf("array from %d to %d:",low,pi-1);
  printIntArray(arr +low,pi-low);
  printf("\n");

  //recursive call on the right side of pivot
  quickSort(arr,pi+1,high);
  printf("Array from %d to %d: ", pi + 1, high);
  printIntArray(arr + pi + 1, high - pi);
  printf("\n");

...
int partition(int arr[], int low, int high){
	...
	int pivot = arr[high];
	// Selecting a pivot point, in this case, the pivot point is the last element, high
	int i = (low - 1);
	// Initializing partition, giving i a value of low - 1 means the partition starts with the first element 
	int temp;
	// a temp value used to swap values 
	int store;
	// another temp value used to store where the pivot point should go at the end

	for (int j = low; j <= high - 1; j++){
	// j is set between the first element in the arr and the second last one in the arr and will iterate through the arr to the right
		if (arr[j] <= pivot){
		// condition for swap, if the element is smaller than the value of pivot point
			i++;
			// the partition will move to one unit on the right in each iertation through each swap
	...
	arr[i + 1] = arr[high];
	arr[high] = store;
	// swap the pivot point the right point, which swaps the element on the right of partition with the pivot point. In this case, all elements on 
	// the left of pivot point are smaller than the value of pivot point and all the elements on the right are larger than the value of pivot point 

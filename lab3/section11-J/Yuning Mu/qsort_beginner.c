...
// Name: partition
// Input(s):
//  - 'arr' is a given array which needs to be further partitioned
//  - 'low' is the starting index of the subarray
//  - 'high' is the ending index of the subarray
// Output: the new index of the pivot after rearranging the subarray
int partition(int arr[], int low, int high) {
    // Choose the last element in the subarray as the pivot
    int pivot = arr[high];
    // i is the index to put in the next smaller value
    int i = low - 1;
    // iterate through the whole subarray
    for (int j = low; j < high; j++) {
        // If the current number is smaller than or equal to the pivot
        if (arr[j] <= pivot) {
            // we move the index one step forward, to get prepared for arranging the next smaller value
            i++;
            // Put the smaller value into the correct position, by swapping the two numbers
            // save the current value of arr[i]
            int temp = arr[i];
            // change the value of arr[i] into arr[j]
            arr[i] = arr[j];
            // change the value of arr[j] into the temp value saved
            arr[j] = temp;
        }
    }
    // After arranging all the smaller values to the left, place the pivot after them.
    // save the current value of arr[i + 1]
    int temp = arr[i + 1];
    // change the value of arr[i + 1] into arr[high]
    arr[i + 1] = arr[high];
    // change the value of arr[high] into the temp value saved
    arr[high] = temp;
...
// Name: quickSort
// Input(s):
//  - 'arr' is the array to perform quick sort on
//  - 'low' is the starting index to perform quick sort
//  - 'high' is the ending index to perform quick sort
// Output: No value is returned, but 'arr' should be modified to store a sorted array of numbers.
void quickSort(int arr[], int low, int high) {
    if (low < high) {
        int previousPivotIndex = partition(arr, low, high);
        quickSort(arr, low, previousPivotIndex - 1);
        quickSort(arr, previousPivotIndex + 1, high);
    }
}

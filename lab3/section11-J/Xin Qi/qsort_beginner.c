...
int partition(int arr[], int low, int high) {
    ...
    for (int j = low; j < high; j++) {
        if (arr[j] <= pivot) { // if the element is smaller than or equal to pivot
            ...
            int temp = arr[i]; // use temp to store the value of arr[i]
            arr[i] = arr[j]; // pass the value of arr[j] to arr[i]
            arr[j] = temp; // pass temp, the initial value of arr[i] to arr[j]
        }
    }
...
// =============== Sort Function ===============
// Quick sort function, implement quick sort recursively to an array
// within the range. It chooses a pivot and partitions the array.
// The process will be repeated until the base case is reached, which means
// there is 1 or 0 elements in the subarrays.
// Name: quickSort
// Inputs:
// - 'arr[]' an array of integers
// - 'low' an integer indicating the first element in the array for partition.
// - 'high' an interger indicating the last element in the array for partition.
// Output:
// - nothing
void quickSort(int arr[], int low, int high) {
    // Check if the range from low to high contains more than 1 element
    if (low < high) {
    ...
    // do not need to sort if there is only 1 element left, nothing to implement
    // end the sorting process
}
...
void sortIntegers(int* array, unsigned int size) {
    ...
    // Call quickSort function to implement quick sort

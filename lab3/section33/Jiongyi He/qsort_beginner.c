...
//  - 'high' is hte last index of the array.
...
int partition(int arr[], int low, int high) {
    ...
    for (int j = low; j < high; j++) { // Loop through the array
        if (arr[j] <= pivot) {  // If current element is smaller than or equal to pivot
            ...
            // Swap arr[i] with arr[j] using array notation
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
        }
...
//  - 'array' is a pointer to an integer address.
//  - 'low' is the first index of the array.
//  - 'high' is the last index of teh array.
//  - Output: Null.
...
void sortIntegers(int* array, unsigned int size) {      
        ...
        quickSort(array, 0, size - 1); // Call quickSort with the full range of the array    

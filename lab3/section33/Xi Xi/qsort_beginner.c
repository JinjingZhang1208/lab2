...
int partition(int arr[], int low, int high) {
    ...
    int i = low - 1; // set the index of the smaller element to the left of the pivot
    ...
    int temp = arr[i + 1]; // swap the elements at i + 1 and high
    arr[i + 1] = arr[high];
    arr[high] = temp;
...
void quickSort(int arr[], int low, int high) {
    if (low < high) {
        ...
        quickSort(arr, low, pi - 1); // sort the left side of the array
        quickSort(arr, pi + 1, high); // sort the right side of the array
    }
}

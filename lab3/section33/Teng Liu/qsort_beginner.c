...
int partition(int arr[], int low, int high) {
    ...
    int pivot = arr[high];
    // initializes a counter i at index 0.
    int i = low;

    // j is for iterating the elements through the array.
    int j;
    for (j = low; j < high; j++) {
        // if current element is smaller than the pivot.
        if (arr[j] < pivot) {
            // move the current element to the front of the pivot.
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
            // i is incremented to move to the next position.
            i++;
        }      
    }
    // move the pivot to index i to make sure it's in the appropriate position.
    int temp = arr[i];
    arr[i] = arr[high];
    arr[high] = temp;
    return i;
...
void quickSort(int arr[], int low, int high) {
    ...
    if (low < high) {
        ...
        // sort the first part of the array(smaller subarray)
        quickSort(arr, low, pivot - 1);
        // sort the second part of the array(larger subarray)

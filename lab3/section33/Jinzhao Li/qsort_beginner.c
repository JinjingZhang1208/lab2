...
// low is not the smallest element, but the element number at the beginning of the array.
// high is not the largest element, but the element number at the end of the array.
int partition(int arr[], int low, int high) {
    int pivot = arr[high];    // Select the last element of the array as pivot
    // i is the index used to track the most recently found element smaller than the pivot
    int i = low - 1;

    for (int j = low; j < high; j++) {   // Iterate over array except pivot element
        if (arr[j] < pivot) {   // If the current element is smaller than the pivot
            i++;   // i keeps track of the number of elements smaller than the pivot
    ...
    // making sure that everything on the left is less than it and
    // everything on the right is greater than or equal to it.
    int temp = arr[i + 1];
    arr[i + 1] = arr[high];
    arr[high] = temp;

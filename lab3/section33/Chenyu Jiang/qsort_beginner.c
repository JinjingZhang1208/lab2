...
//Name: partition
//Inputs:
// - 'arr[]' is a opinter to the first element in the array.
// - 'low' is the lowest index of the array.
// - 'high' is the highest index of the array. In this case ,it would be size - 1
// the algorithm pick the element at the very end to be the pivot.
//Return: the position from where the 
int partition(int arr[], int low, int high) {
    // pick the element at the very end of the array to be the pivot.
    int pivot = arr[high];
    // create i, a pointer for greater element.
    int i = low - 1;
    // iterate through all numbers and compare each with the pivot.
    for (int j = low; j < high; j++) {
        // if the number is smaller than the pivot,
        if (arr[j] <= pivot) {
            // increase i by one,
            i++;
            // create a temporary int variable called swap to temporarily
            // store the number indexed at j in the array,
            int swap = arr[i];
            // assign the number indexed at j to i in the array,
            arr[i] = arr[j];
            // assign the number stored in the swap to index j in the array.
            arr[j] = swap;
         }
     }
     // create a temporary int variable called swap to temporarily
     // store the number indexed at i + 1 in the array,
     int swap = arr[i + 1];
     // assign the pivot number to the array at index 1 + 1,
     arr[i + 1] = arr[high];
     // assign the number stored in the swap variable to the very end of the array.
     arr[high] = swap; 
     // return i + 1
     return i + 1;
}

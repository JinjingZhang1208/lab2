...
//     This is the start of some 'contiguous block of memory' that we will partition.
//   - 'low' is the starting index of the partition.
//   - 'high' is the ending index of the partition.
// Output: Returns the index of the pivot after partitioning.
int partition(int arr[], int low, int high) {
    int pivot = arr[high]; // Pivot element (choose the last element as pivot)
    int i = low - 1;       // Index of smaller element, initialized to one less than the start

    // Iterate through the elements from 'low' to 'high - 1'
    for (int j = low; j <= high - 1; j++) {
        // If the current element is less than or equal to the pivot
        if (arr[j] <= pivot) {
            i++; // Move the index of the smaller element to the right
            // Swap arr[i] and arr[j] to maintain the partitioning condition
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
        }
...
//   - 'low' is the starting index of the array or partition.
//   - 'high' is the ending index of the array or partition.

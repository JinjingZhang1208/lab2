...
int partition(int arr[], int low, int high) {
    // chooses pivot as the highest index in the array
    int pivot = arr[high];
    // ensures that i is at -1 index (before the beginning of the array)
    // you want it to not point at anything before the first iteration starts
    // once the first iteration starts, THEN you would start pointing at the elements
    // that need to be swapped
    int i = low - 1;
    // iteration starts with index 1 (second element)
    // continues until it reaches the second-highest index (element before the pivot)
    for (int j = low; j < high; j++) {
        // if selected element before pivot is smaller than pivot, it runs the swap function
        // if selected element is greater, then it does not swap in this iteration
        if (arr[j] < pivot) {
            // moves i to the next index
            i = i + 1;
            // swaps element at index i with element at index j
            // element i is currently pointing at the smallest right most element
            // if element j is smaller than element i, then it swaps
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
        }
    }
    // swaps the pivot element (at index 'high') with the element next to 'i'
    // places the pivot element at its correctly sorted position
    int temp = arr[i + 1];
    arr[i + 1] = arr[high];
    arr[high] = temp;
    // returns index of the pivot element before partitioning
    return i + 1;
}

...
int partition (int arr[], int low, int high) {
    int pivot = arr[high]; //Use the last element of the subarray as a pivot.
    int i = low - 1; //i will mark the end of the "less than pivot" section.
//Iterate through each element to see if it needs to be rearranged.
    for (int j = low; j < high; j++) {
        if (arr[j] <= pivot) { //If the current element is less than or equal to the pivot
            i = i + 1;
            //It should be located to the left of the pivot with i incremented.
            int temp = arr[i]; 
            // Swap arr[i] and arr[j] in order to move the smaller element to the left side.
            arr[i] = arr[j];
            arr[j] = temp;
        }
    }
//Exchanges the pivot element with the first element of the "greater than pivot" section.
...
void quickSort(int arr[], int low, int high) {
    if (low < high) { //make sure the subarray has more than one element.
        int pivotIndex = partition(arr, low, high);// Call the partition function to partition and get the index of the pivot.
        quickSort(arr, low, pivotIndex - 1); //Recursively sort the subarray to the left of the pivot.
        quickSort(arr, pivotIndex + 1, high);
        // Recursively sort the subarray to the rightof the pivot.
    }
}
...
void sortIntegers(int* array, unsigned int size) {
    ...
    quickSort(array, 0, size - 1);// quicksort of the whole array.
}

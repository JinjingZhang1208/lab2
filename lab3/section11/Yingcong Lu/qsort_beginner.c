...
// Name: partition
// Inputs:
//  - 'arr[]': Array of integers to be partitioned.
//  - 'low': Starting index of the portion of the array
//  - 'high': Ending index of the portion of the array
// Output: Returns the sorted index of the pivot element 
int partition(int arr[], int low, int high) {
    int pivot = arr[high]; // Choose the last elements as pivot
    int i = (low - 1); // i is the slow pointer which dividing the smaller elements

    for (int j = low; j <= high - 1; j++) { // j is the quicker pointer which dividing the larger elements
        if (arr[j] <= pivot) { // If an element is smaller than the pivot, execute the swap process
            i++; // Increment the index of the smaller element
            // Swap arr[i] and arr[j], then smaller elements and larger elements are seperated
            int temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
        }
    }
    // when the loop finished, i means the last elements smaller than the pivot
    // so i+1 can represent the position of the pivot
    // swap pivot and place it in the right position
    int temp = arr[i + 1];
    arr[i + 1] = arr[high];
    arr[high] = temp;

    return i + 1; // Return the index of the pivot which is important for the recursion process
}

// Name: quickSort
// Inputs:
//  - 'arr[]': Array of integers
//  - 'low': Starting index of part of the array
//  - 'high': Ending index of part of the array
// Output: No value is returned, but 'array' should be modified to store a sorted array of numbers
void quickSort(int arr[], int low, int high) {
    //Base case
    if (low < high) {
        // return the index of pivot and seperate smaller and larger partition
        int q = partition(arr, low, high);

        // Recursion process that sort both smaller partition
        // and larger partition

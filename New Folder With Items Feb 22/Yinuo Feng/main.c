...
//This unit test tests the edge case for removeKey -- when the hashmap is null.
int unitTest3() {
    //When the hashmap is null
    hashmap_t* myStocks = NULL;
    hashmap_printKeys(myStocks);
    hashmap_removeKey(myStocks, "BMO");
    hashmap_printKeys(myStocks);
    
    //When the removed key is not found
    myStocks = hashmap_create(5);
    hashmap_insert(myStocks, "BNS", 66);
    hashmap_printKeys(myStocks);
    hashmap_removeKey(myStocks, "BMO");
    printf("After removing the nonexisting key, the hashmap looks like:\n");
    hashmap_printKeys(myStocks);
    hashmap_delete(myStocks);
    return 1;
}

...
void hashmap_removeKey(hashmap_t* _hashmap, char* key) {
    ...
    unsigned int bucket = yourHash(key, _hashmap->buckets);
    node_t* n = _hashmap->arrayOfLists[bucket];
    node_t* n1;
    //  Case 1: the node that needs to be removed is the head node.
    if (strcmp(n->kv->key, key) == 0 && n != NULL) {
        _hashmap->arrayOfLists[bucket] = n->next;
        free_node(n);
        return;
    }

    // Case 2: the node that needs to be removed is not the head node.
    while (strcmp(n->kv->key, key) != 0 && n != NULL) {
        n1 = n->next;
        if (strcmp(n1->next->kv->key, key) == 0) {
            n->next = n1->next;
            free_node(n1);
            return;
        }

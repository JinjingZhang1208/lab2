...
void hashmap_removeKey(hashmap_t* _hashmap, char* key) {
    if (_hashmap == NULL || key == NULL) {
        return; // Hashmap or key is null, nothing to do
    }
    ...
    while (current != NULL) {
        if (strcmp(current->kv->key, key) == 0) {
            // Case 1: Node to remove is the head node
            if (prev == NULL) {
                _hashmap->arrayOfLists[bucket] = current->next;
            } else {
                // Case 2: Node to remove is not the head node
                prev->next = current->next;
            }

            // Free the memory allocated for the node
            free(current->kv->key); // Free the key string
            free(current->kv);      // Free the pair
            free(current);          // Finally, free the node itself
            return; // Key has been removed, exit the function
        }
        prev = current;
